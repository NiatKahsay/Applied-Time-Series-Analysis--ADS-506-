...
ili_weight |> 
  features(percent_weighted_ili, c(unitroot_kpss, 
...
# Use original ili_weight data so fable 
# can automatically back-transform metric data 
ili_trn <- ili_weight |> filter(week_end <= cutoff)
ili_val <- ili_weight |> filter(week_end > cutoff)

# Create a modeling function
model_ili <- function(data) { data |> 
  model(
    # Auto ARIMA model 
...
```{r}
# Create a model function that contains the 
# best candidates for forecasting and Baseline model
ili_fit_candidates_fn <- function(data) {
  data |>
    model(
      # TSLM candidate
      TSLM_Model = TSLM(box_cox(percent_weighted_ili, 
                                lambda_optimal) ~ pandemic 
                        + trend() + fourier(K = 20, period 
                                            = 52)),
      # Auto ARIMA candidate
      ARIMA_auto = ARIMA(
        box_cox(percent_weighted_ili, lambda_optimal) ~ 
          pandemic + season(period = 52)
      ),
      # SNAIVE-equivalent ARIMA
...
ili_fit_candidates <- ili_fit_candidates_fn(ili_trn)
...
acc_52wk <- forecast_52wks |>
  ...
  tab_header(
    ...
    subtitle = ("Forecast performance over 
                52-week horizon with COVID-19 period **adjusted** 
                (ARIMA and TSLM)"))
...
*CV was not running with COVID-19 dummy variable. Check if the same patterns exisit without the pandemic signal. Is the model ranking 
